...
def merge_monster(config_path):
    ...
    if 'device' in config and isinstance(config['device'], list):
        devices = config['device']
    else:
        devices = ['cuda:0', 'cuda:1', 'cuda:2', 'cuda:3']  # Default to 4 GPUs if not specified
    ...
    with torch.no_grad():
        ...
        # Explicitly set device for torch operations - modified for multi-GPU
        torch.set_default_device('cuda:0')  # Use the first GPU as default for operations

        # Setting all the seeds - ensure reproducibility across multiple runs
        torch.manual_seed(random_seed)
        random.seed(random_seed)
        torch.cuda.manual_seed_all(random_seed)  # For all GPUs

        # Ensure deterministic behavior
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False

        # Load the base model + tokenizer with multi-GPU support
        model1 = load_model(model_path1, 'cpu')  # Load on CPU first to avoid CUDA memory spike
        model1name = model_path1.split('/')[-1]

        # If multiple GPUs are configured, wrap the model with DataParallel
        if len(devices) > 1:
            model1 = model1.to('cuda:0')  # Move to the first GPU
            model1 = DataParallel(model1, device_ids=[0, 1, 2, 3])  # Assuming 4 GPUs
        else:
            model1 = model1.to(devices[0])  # Single device specified, move model to that device
